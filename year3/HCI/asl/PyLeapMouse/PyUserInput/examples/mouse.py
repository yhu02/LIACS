...
with daemon.DaemonContext():
	...
	try:
		...
		class event(PyMouseEvent):
			...
			FORMAT = '%(asctime)-15s ' + gethostname() + \
						' touchlogger %(levelname)s %(message)s'
			logging.basicConfig(filename='/var/log/mouse.log',
			...
			if press:
				logging.info('{ "event": "click", "type": "press", "x": "' +
							str(x) + '", "y": "' + str(y) + '"}')
			else:
				logging.info('{ "event": "click", "type": "release", "x": "' +
							str(x) + '", "y": "' + str(y) + '"}')
