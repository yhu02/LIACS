      ...
      for (int x = 0; x < background->width; x += block * blockWidth)
        {
          for(int blockRow = y; blockRow < y + block; blockRow++){
            for(int blockCol = x; blockCol < x + block * blockWidth; blockCol++){
              ...
              int tx = blockCol % tile->width;
              int ty = blockRow % tile->height;
              rgba_t dst, src;
              
              RGBA_unpack(dst, *image_get_pixel(background, blockCol, blockRow));
              ...
              RGBA_pack(*image_get_pixel(background, blockCol, blockRow), dst);
            }
          }
        }
    }
