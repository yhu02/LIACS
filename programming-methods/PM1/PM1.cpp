...
int main(){
	...
	std::cout << "bereken het product van de volgende twee getallen om verder te gaan" << std::endl;
	std::cout << " " << getal1 << std::endl << " " << getal2 << " x" << std::endl << " --" << std::endl;
	if(getal1 % 10 == 0 || getal2 % 10 == 0){
		std::cin >> tempx;
	}
	else{
		std::cin >> tempx >> tempy;
	}
	tempresult = tempx + tempy;
	//Controleer of het antwoord binnen de marges liggen van het werkelijke antwoord + epsilon
	if(!(tempresult > round(antwoord3 * (1 + epsilon)) || tempresult < round(antwoord3 * (1 - epsilon)))){	
		std::cout << "Het antwoord ligt binnen de grenzen. Het antwoord is: " << antwoord3 <<std::endl << "Je hebt geantwoord: " << tempresult << std::endl;
		std::cout << "Gefeliciteerd! ";
		resultaatjaar >= 30 ? std::cout << "U ": std::cout << "Je ";
		std::cout << "bent toegelaten tot een beta-studie!" << std::endl;
	}//Sectie 4
	else {
		std::cout << "Het antwoord ligt buiten de grenzen. Het antwoord is: " << antwoord3 <<std::endl << "Je hebt geantwoord " << tempresult << std::endl;
		resultaatjaar >= 30 ? std::cout << "U ": std::cout << "Je ";
